...
$s = oci_parse($c, "select * from comenzi order by DATA_COMANDA");

oci_execute($s);

echo <<<EOD
	<h2> Sortarea tabelului COMENZI dupa DATA_COMANDA </h2>
	...
	<br>
	<h3> Tabelul COMENZI sortat crescator, dupa DATA_COMANDA: </h3>
EOD;
...
$s = oci_parse($c, "select *from COMENZI order by DATA_COMANDA desc");
oci_execute($s);


echo "<h3> <br> Tabelul COMENZI sortat descrescator, dupa DATA_COMANDA: </h3>";
echo "<table border='1'>\n";
$ncols = oci_num_fields($s);
echo "<tr>\n";
for ($i = 1; $i <= $ncols; ++$i) {
