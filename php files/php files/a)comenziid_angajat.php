...
$s = oci_parse($c, "select * from comenzi order by ID_ANGAJAT");

oci_execute($s);

echo <<<EOD
	<h2> Sortarea tabelului COMENZI dupa ID_ANGAJAT </h2>
	...
	<br>
	<h3> Tabelul COMENZI sortat crescator, dupa ID_ANGAJAT: </h3>
EOD;
...
$s = oci_parse($c, "select *from COMENZI order by ID_ANGAJAT desc");
oci_execute($s);


echo "<h3> <br> Tabelul COMENZI sortat descrescator, dupa ID_ANGAJAT: </h3>";
echo "<table border='1'>\n";
$ncols = oci_num_fields($s);
echo "<tr>\n";
for ($i = 1; $i <= $ncols; ++$i) {
