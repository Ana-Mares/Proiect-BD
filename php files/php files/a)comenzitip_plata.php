...
$s = oci_parse($c, "select * from comenzi order by TIP_PLATA");

oci_execute($s);

echo <<<EOD
	<h2> Sortarea tabelului COMENZI dupa TIP_PLATA </h2>
	...
	<br>
	<h3> Tabelul COMENZI sortat crescator, dupa TIP_PLATA: </h3>
EOD;
...
$s = oci_parse($c, "select *from COMENZI order by TIP_PLATA desc");
oci_execute($s);


echo "<h3> <br> Tabelul COMENZI sortat descrescator, dupa TIP_PLATA: </h3>";
echo "<table border='1'>\n";
$ncols = oci_num_fields($s);
echo "<tr>\n";
for ($i = 1; $i <= $ncols; ++$i) {
